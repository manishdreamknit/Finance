...
#    print sSoapAction
    ...
    try:
        ...
        if e.code == 404:
            ...
            f.write(e.read())
            f.close()
            raise ErrorOccured("URL Error occured while sending request: " + e.read())
        elif e.code == 401:
            StepResult = "Executed"
            LogToExcel(FileName,RowNo,"Send_Request_Result",StepResult)         # calling function to write result of step to excel file
            print e.read()
        ...
        raise ErrorOccured("Unknown error occured." )
...
def Send_Request_WithRequestXML(WebService,url,sSoapAction,sRequest,Username,Password,FolderPath):
    ...
    #except e:
        ...
        ErrorText = str(e)
        print ErrorText
        raise ErrorOccured("Unknown error occured." + ErrorText )
...
def Send_Get_Request(url, Username, Password):
    ...
    SResponseCode = response.getcode()
    print SResponseCode
    SResponseText = response.read() 
    print SResponseText

    return SResponseCode,SResponseText
...
def Send_TPP_Request(url,sRequest,Username,Password,FolderPath):
    ...
    headers = {'Content-Type': 'application/x-www-form-urlencoded',
                'Authorization': 'Basic YmFycG9zdDo4Z2d2cWh2cA=='
               }
